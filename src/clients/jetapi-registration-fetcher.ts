...
function genUS(): string {
  // N + 1-5 chars, mix letters/numbers
...
function genUK(): string {
  // G- followed by 4 alnum
  const chars = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789';
  let s = 'G-';
...
function genCA(): string {
  // C-Fxxx or C-Gxxx
  const letters = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ';
  const prefix = Math.random() < 0.5 ? 'C-F' : 'C-G';
  let s = prefix;
  for (let i = 0; i < 3; i++) s += letters.charAt(Math.floor(Math.random() * letters.length));
  return s;
}

function genAU(): string {
  // VH-XXX
  const letters = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ';
  let s = 'VH-';
  for (let i = 0; i < 3; i++) s += letters.charAt(Math.floor(Math.random() * letters.length));
...
function genFR(): string {
  // F-XXXX
  const letters = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ';
  let s = 'F-';
  for (let i = 0; i < 4; i++) s += letters.charAt(Math.floor(Math.random() * letters.length));
  return s;
}

function genIT(): string {
  // I-XXXX
  const letters = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ';
  let s = 'I-';
  for (let i = 0; i < randInt(3,4); i++) s += letters.charAt(Math.floor(Math.random() * letters.length));
  return s;
}
...
function genBR(): string {
  // PR-ABC / PP-ABC / PT-ABC pattern for Brazil
  const prefixes = ['PR-', 'PP-', 'PT-', 'PU-'];
  const letters = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ';
...
function genJP(): string {
  // JAxxx or similar
  const chars = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789';
  let s = 'JA';
  for (let i = 0; i < randInt(3,4); i++) s += chars.charAt(Math.floor(Math.random() * chars.length));
  return s;
}

function genNL(): string {
  // PH-xxx (Netherlands)
  const letters = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ';
  let s = 'PH-';
  for (let i = 0; i < 3; i++) s += letters.charAt(Math.floor(Math.random() * letters.length));
...
const generators: Array<() => string> = [
  ...
  genUK, genUK,
  genDE, genFR, genCA, genAU,
  genIT, genAT, genBR, genJP, genNL,
];
...
function tryExtractImagesFromJson(json: any): Array<{ url: string; photographer?: string; link?: string; id?: string }> {
  ...
  if (json.image || json.url || json.photo) {
  ...
  for (const c of candidates) {
    ...
    const urlCandidates = [
      ...
      (c.urls && c.urls.full), (c.urls && c.urls.large), (c.urls && c.urls.original),
      (c.urls && c.urls.fullsize)
    ];
