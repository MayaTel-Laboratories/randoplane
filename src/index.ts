...
const DEFAULT_JETAPI_BASE = 'http://127.0.0.1:4000';
...
function buildAltTextFromImage(regOrKeyword: string, img: JetPhotosImage | null) {
...
async function chooseRandomImageFromJetPhotos(jp: { Reg?: string; Images?: JetPhotosImage[] } | undefined) {
  if (!jp || !Array.isArray(jp.Images) || jp.Images.length === 0) return null;
  const withAttr = jp.Images.filter((i) => i.Photographer && i.Link);
  if (withAttr.length > 0) return pickRandom(withAttr);
  return pickRandom(jp.Images);
}
...
async function runOnce() {
  ...
  const base = process.env.JETAPI_BASE || DEFAULT_JETAPI_BASE;
  ...
  console.log(`Querying JetAPI at ${base} for keyword="${keyword}" photos=${photos}`);
  const jp = await fetchJetPhotos(keyword, photos, base);
  const chosenImage = await chooseRandomImageFromJetPhotos(jp);

  if (!chosenImage) {
    throw new Error(`JetAPI returned no images for keyword "${keyword}"`);
  }
